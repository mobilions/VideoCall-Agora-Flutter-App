...
class _MyAppState extends State<MyApp> {
  ...
  bool _isConfigInvalid() {
    return config.appId == '<YOUR_APP_ID>' || config.token == '<YOUR_TOKEN>' || config.channelId == '<YOUR_CHANNEL_ID>';
  }
      ...
      itemBuilder: (context, index) {
        return _data[index]['widget'] == null
            ? Ink(
                ...
                child: ListTile(
                  title:
                      Text(_data[index]['name'] as String, style: const TextStyle(fontSize: 24, color: Colors.white)),
                ),
              )
            : ListTile(
                onTap: () {
...
class _WebSetupPage extends StatefulWidget {
  const _WebSetupPage({Key? key, required this.setupCompleted}) : super(key: key);
...
class _WebSetupPageState extends State<_WebSetupPage> {
  ...
  void _validCheck() {
    _isValid = _appIdController.text.isNotEmpty && _channelIdController.text.isNotEmpty;
    setState(() {});
  }
            ...
            ElevatedButton(
              onPressed: !_isValid
                  ...
                  : () {
                      ...
                      _configOverride.set(keyChannelId, _channelIdController.text);
